...
async function analyzeFile(file){
  ...
  const statement=await postJSON("/api/extract",payload); state.statement=statement; doneStep("extract");
  ...
  const cls=await postJSON("/api/classify",{transactions:debits,merchantMemory:state.memory,webLookup:$("#webLookup").checked});
  ...
  renderDashboard();
  try{ state.insights=await postJSON("/api/insights",{summary:summaryForAI()}); renderInsights(); }catch{ renderFallbackInsights(); }
...
async function postJSON(url,body){
  ...
  const r=await fetch(url,{method:"POST",headers:{"Content-Type":"application/json","X-OpenAI-API-Key":state.apiKey},body:JSON.stringify(body)});
